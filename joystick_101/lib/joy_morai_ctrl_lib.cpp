...
JoyMoraiCtrl::JoyMoraiCtrl(const ConfigParam& cfg) : cfgParam_(cfg), joyXbox360_(cfg), it_(nh_)
{
...
{
  // using rosbag-based imglog mode only
  if ((bool)(msgImgLogCmd_.data))
  {
    // logging imgdb data, usleep(in micro second, 10^-6)
    usleep(cfgParam_.nLogDuration);
    GenShutterAutoCmd();
    ROS_INFO("MoraiImgDBlogSize:(%d)", nDbCurrLogSize_);
    usleep(cfgParam_.nLogDuration);
    nDbCurrLogSize_++;
  }
  else  // using joystick control with the shutter command
  {
    // resetting the curr log size
    nDbCurrLogSize_ = 0;
    ...
    msgPubMoraiSimCtrlCmd.steering =
        (-1.0f) * (cfgParam_.moraiSimCtrlScale.fSteer) * (joyMoraiSimCtrlCmd_.ctrlMove.fYaw);
