...
JoyPS4BT::JoyPS4BT(const ConfigParam& cfg) : cfgParam_(cfg)
{
  // generating subscriber, joy node
  subJoyRaw_ = nh_.subscribe<sensor_msgs::Joy>(cfgParam_.strJoyTpNm, 1, &JoyPS4BT::CbJoyRaw, this);
...
void JoyPS4BT::CbJoyRaw(const sensor_msgs::Joy::ConstPtr& msgRaw)
{
  joyCurrRaw_ = *msgRaw;
...
JoyCtrlCmd JoyPS4BT::GenJoyInfoLoop()
{
  ...
  joyCtrlRef_ = GenJoyCtrlAxisRefInfo(cfgParam_.joyCfgPS4BT, joyCurrRaw_, joyCtrlRef_);
  joyCtrlRef_ = GenJoyCtrlBtnsRefInfo(cfgParam_.joyCfgPS4BT, joyCurrRaw_, joyCtrlRef_);
  return joyCtrlRef_;
}

// generating the joystick control reference information, buttons
JoyCtrlCmd JoyPS4BT::GenJoyCtrlBtnsRefInfo(JoyRaw param, sensor_msgs::Joy joyRaw, JoyCtrlCmd res)
{
...
JoyCtrlCmd JoyPS4BT::GenJoyCtrlAxisRefInfo(JoyRaw param, sensor_msgs::Joy joyRaw, JoyCtrlCmd res)
{
