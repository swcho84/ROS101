...
def set_dialect(dialect):
    ...
    try:
        modname = 'Python.mavlink_nearthlab'
...
class mavwebsocket(mavfile):
    ...
    def __init__(self, address, port, source_system=255, retries=3, use_native=default_native):
        self.port = port
        self.address = address
        ...
        self.th = threading.Thread(target=self.ws.run_forever)
        self.th.daemon = True
        self.th.start()
        mavfile.__init__(self, None, self.url, source_system=source_system, use_native=use_native)
        self.buf = Queue(1000)
    ...
    def write(self, buf):
        try:
            ...
            print("Websocket write error: " + e.message)
